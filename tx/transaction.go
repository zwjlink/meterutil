...
func (t *Transaction) Hash() (hash meter.Bytes32) {
...
func (t *Transaction) EvaluateWork(origin meter.Address) func(nonce uint64) *big.Int {
	hw := meter.NewBlake2b()
...
func (t *Transaction) Origin() (meter.Address, error) {
	...
	origin := meter.Address(crypto.PubkeyToAddress(*pub))
...
func (t *Transaction) DelegatorSigningHash(origin meter.Address) (hash meter.Bytes32) {
	hw := meter.NewBlake2b()
	hw.Write(t.SigningHash().Bytes())
	hw.Write(origin.Bytes())
	hw.Sum(hash[:0])
...
func (t *Transaction) Delegator() (*meter.Address, error) {
	if err := t.validateSignatureLength(); err != nil {
		return nil, err
	}
	...
	delegator := meter.Address(crypto.PubkeyToAddress(*pub))
...
func (t *Transaction) DecodeRLP(s *rlp.Stream) error {
	...
	*t = Transaction{body: body}
	...
	t.cache.size.Store(meter.StorageSize(rlp.ListSize(size)))
	return nil
}
...
func (t *Transaction) Size() meter.StorageSize {
	if cached := t.cache.size.Load(); cached != nil {
		return cached.(meter.StorageSize)
	}
...
func (t *Transaction) ProvedWork(headBlockNum uint32, getBlockID func(uint32) (meter.Bytes32, error)) (*big.Int, error) {
	ref := t.BlockRef()
	refNum := ref.Number()
